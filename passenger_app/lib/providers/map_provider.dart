...
class MapProvider extends ChangeNotifier {
  ...
  void _startListening() {
    _busSubscription = _firestore.collection('buses').doc(busId).snapshots().handleError((error) {
      if (error is FirebaseException) {
        debugPrint('Firestore Stream Error (map_provider): ${error.code}');
      } else {
        debugPrint('Unknown Stream Error (map_provider): $error');
      }
    }).listen(
      (snapshot) {
        try {
          ...
          debugPrint('Data Parsing Error (map_provider): $e');
          busData = null;
          notifyListeners();
        }
      },
      onError: (error) {
        debugPrint('Error listening to bus snapshot: $error');
      }
    );
  }
